...
CODE_VERSION = "v1.1-schedules-fallback"  # shows in app so you know the build is current

# Try both .gz and plain CSV (nflverse sometimes shifts paths)
SCHEDULE_URLS = [
...
# Stable pattern for PBP parquet
PBP_PARQUET_TMPL = (
...
def fetch_schedule() -> pd.DataFrame:
    """
    Try multiple schedule URLs. Return DataFrame with expected columns.
    """
    last_err = None
    for url in SCHEDULE_URLS:
        try:
            ...
            if not set(cols).issubset(df.columns):
                raise ValueError(f"Schedule file missing required columns: {set(cols)-set(df.columns)}")
            return df[cols]
...
def compute_team_unit_metrics(pbp: pd.DataFrame, season: int, week: int):
    """
    Build offense_units and defense_units from PBP up to selected week (REG).
    ...
    deff["coverage_grade"] = np.nan  # placeholder
