...
CODE_VERSION = "v1.2-schedules-espn-fallback"

# Primary (nflverse) + secondary (ESPN) sources for schedules
SCHEDULE_URLS = [
...
# Stable pattern for PBP parquet by season (nflverse releases)
PBP_PARQUET_TMPL = (
...
def _fetch_schedule_espn_full_season(season: int) -> pd.DataFrame:
    ...
    Endpoint example:
      https://site.api.espn.com/apis/v2/sports/football/nfl/scoreboard?week=3&seasontype=2&dates=2025
    """
    rows = []
    for wk in range(1, 19):
        ...
        try:
            ...
            # If one week fails, continue; we'll just have fewer rows
            continue
        for ev in data.get("events", []):
            ...
            gameday = ev.get("date")  # ISO timestamp; Streamlit displays fine
...
def fetch_schedule() -> pd.DataFrame:
    """
    Try nflverse first; on failure, return a DataFrame combining ESPN REG-season
    weeks 1..18 for the current sidebar-selected season (set by streamlit_app).
    """
    # We try nflverse (all seasons). If that fails, we can't know which season the
    # caller wants unless they filter later, so we also provide an ESPN-based helper
    # that the app can filter by the selected season.
    try:
        return _fetch_schedule_nflverse()
    except Exception:
        # NOTE: streamlit_app filters by the selected season after calling this,
        # so returning an empty frame here would break UX. Instead, return an empty
        # shell; the app calls a season-specific helper below when empty.
...
def fetch_schedule_for_season_from_espn(season: int) -> pd.DataFrame:
    """Public helper: build a REG schedule for a single season via ESPN."""
...
def compute_team_unit_metrics(pbp: pd.DataFrame, season: int, week: int):
    """
    Offense units from posteam; defense units from defteam.
    Includes OL/DL proxies built from sack- and stuff-rates.
    """
    df = pbp.copy()
    ...
    for _, r in ol.iterrows():
        offense_units.append({"team":r["team"],"unit":"OL","epa_per_play":np.nan,"success_rate":np.nan,
                              "explosive_rate":np.nan,"pass_block_win":r["pass_block_win_proxy"],
                              "run_block_win":r["run_block_win_proxy"]})
